...
public interface CustomerRegistrationValidator extends Function<Customer, ValidationResult> {
	...
	static CustomerRegistrationValidator isPhoneNumberValid() {
		return customer -> {
			...
			return customer.getPhoneNumber().startsWith("+0") ? SUCCESS : PHONE_NUMBER_NOT_VALID;
		};
	}

	static CustomerRegistrationValidator isAnAdult() {
		return customer -> {
			System.out.println("running isAnAdult validation");
			return Period.between(customer.getDateOfBirth(), LocalDate.now()).getYears() > 16 ? SUCCESS
					: IS_NOT_AN_ADULT;
		};
	}
